...
// version: 22.10.27
// Copyright (C) 2018, 2020, 2021, 2022 Jeroen P. Broks
...
namespace TrickyUnits {
	public class QuickStream {
		...
		public ulong ReadULong() { return BitConverter.ToUInt64(GrabBytes(8), 0); }
		...
		/// <summary>
		/// Similar to ReadString, but where ReadString uses a 32bit int to get the length, this function uses a 64bit int.
		/// </summary>
		/// <returns></returns>
		public string ReadString64() {
			...
			if (l == 0) { l = ReadULong(); }
			var ret = new byte[l];
			for (ulong i = 0; i < l; i++) { ret[i] = ReadByte(); }
			//return Encoding.Default.GetString(ret);
			return Encoding.ASCII.GetString(ret);
		}
		...
		public static void Hello() {
			MKL.Version("Tricky Units for C# - qstream.cs","22.10.27");
